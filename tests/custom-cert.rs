...
fn connect_to_custom_cert_instance_ado() -> Result<()> {
    ...
    rt.block_on(async {
        let mut config = Config::from_ado_string("server=tcp:localhost,1433;IntegratedSecurity=true;TrustServerCertificateCA=certs/customCA.crt")?;
...
fn connect_to_custom_cert_instance_jdbc() -> Result<()> {
    ...
    rt.block_on(async {
        ...
        let mut config = Config::from_jdbc_string(
            "jdbc:sqlserver://localhost:1433;TrustServerCertificateCA=certs{/}customCA.crt",
        )?;
