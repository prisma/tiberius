...
#[derive(Debug)]
pub struct Client {
    inner: tiberius::Client<compat::Compat<net::TcpStream>>,
}

impl convert::From<tiberius::Client<compat::Compat<net::TcpStream>>> for Client {
    fn from(inner: tiberius::Client<compat::Compat<net::TcpStream>>) -> Client {
        Client { inner }
    }
}
